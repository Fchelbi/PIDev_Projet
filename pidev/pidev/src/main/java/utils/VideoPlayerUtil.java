...
public class VideoPlayerUtil {
     ...
     * Only defined ONCE here (removed duplicate that caused compile error).
     */
     ...
     * JavaFX WebView cannot play YouTube natively; shows thumbnail + browser button.
     */
    public static VBox createYouTubePlayer(String url) {
        ...
        if (videoId.isEmpty()) return createErrorMessage("URL YouTube invalide", url);
        ...
        fallback.setStyle("-fx-background-color: #1a1a2e;");
        fallback.setPadding(new Insets(40));

        // Thumbnail via WebView (image only — no video, this works reliably)
        WebView thumbView = new WebView();
        thumbView.setPrefHeight(300);
        thumbView.setPrefWidth(500);
        String thumbHtml = "<!DOCTYPE html><html><head>"
                ...
                + "</a>"
                + "</body></html>";
        thumbView.getEngine().loadContent(thumbHtml, "text/html");

        // Open links in system browser instead of navigating inside WebView
        thumbView.getEngine().locationProperty().addListener((obs, oldLoc, newLoc) -> {
        ...
        Label lblTitle = new Label("🎬 " + videoId);
        lblTitle.setStyle("-fx-text-fill:#b2bec3;-fx-font-size:12px;");

        Button btnOpen = new Button("▶  Regarder sur YouTube");
        btnOpen.setStyle("-fx-background-color:#FF0000;-fx-text-fill:white;"
                + "-fx-font-size:16px;-fx-font-weight:700;-fx-padding:14 35;"
                + "-fx-background-radius:30;-fx-cursor:hand;");
        btnOpen.setOnAction(e -> openInBrowser(watchUrl));

        Label hint = new Label("JavaFX ne supporte pas la lecture YouTube intégrée.\nCliquez pour ouvrir dans votre navigateur.");
        hint.setStyle("-fx-text-fill:#636e72;-fx-font-size:11px;-fx-text-alignment:center;");
        hint.setWrapText(true);

        fallback.getChildren().addAll(thumbView, lblTitle, btnOpen, hint);
        container.getChildren().add(fallback);
        container.setUserData(thumbView);
        return container;
    }
     ...
     * Falls back to WebView HTML5 player if MediaPlayer fails.
     */
