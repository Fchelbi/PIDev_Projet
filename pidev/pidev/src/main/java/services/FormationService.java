...
public class FormationService implements CRUD<Formation> {
    ...
    public void insertOne(Formation formation) throws SQLException {
        String req = "INSERT INTO `formation`(`title`, `description`, `video_url`, " +
                "`category`, `coach_id`) VALUES (?, ?, ?, ?, ?)";
        ...
        ps.setInt(5, formation.getCoachId());
        ps.executeUpdate();
    ...
    public void updateOne(Formation formation) throws SQLException {
        String req = "UPDATE `formation` SET `title`=?, `description`=?, " +
                "`video_url`=?, `category`=?, `coach_id`=? WHERE `id`=?";
        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setString(1, formation.getTitle());
        ps.setString(2, formation.getDescription());
        ps.setString(3, formation.getVideoUrl());
    ...
    /**
     * Get formations assigned to a specific coach
     */
    public List<Formation> selectByCoach(int coachId) throws SQLException {
        List<Formation> list = new ArrayList<>();
        String req = "SELECT * FROM `formation` WHERE `coach_id`=?";
        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setInt(1, coachId);
    ...
    /**
     * Map ResultSet row to Formation object
     */
