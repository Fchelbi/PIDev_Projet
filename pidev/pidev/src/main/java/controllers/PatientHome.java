...
public class PatientHome {
        ...
        new Thread(() -> {
            try {
                ...
                HttpResponse<String> resp = c.send(
                        HttpRequest.newBuilder().uri(URI.create("https://www.affirmations.dev/"))
    ...
    private void loadAdvice() {
        ...
        new Thread(() -> {
            try {
                HttpClient c = HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(6)).build();
                HttpResponse<String> resp = c.send(
                        HttpRequest.newBuilder().uri(URI.create("https://api.adviceslip.com/advice"))
                                .timeout(Duration.ofSeconds(8)).header("User-Agent","EchoCare/1.0").GET().build(),
                        HttpResponse.BodyHandlers.ofString());
    ...
    // ════════════════════════════════════════════════════════════════════
    //  SUB-PAGE LOADER — FIX: passes currentUser to every sub-page
    // ════════════════════════════════════════════════════════════════════

    /**
     * Charge une sous-page FXML et injecte currentUser dans son contrôleur.
     * Supporte PatientController (interface) et toute classe avec setUser(User).
     */
    private void loadSubPage(String fxmlName, String title) {
        try {
            ...
            // ── FIX: injecter l'utilisateur dans le contrôleur ────────────
            if (ctrl instanceof PatientController pc) {
                pc.setUser(currentUser);
            }
            // Fallback par réflexion pour les contrôleurs qui n'implémentent pas l'interface
            else if (ctrl != null) {
                try {
                    ctrl.getClass().getMethod("setUser", entities.User.class).invoke(ctrl, currentUser);
                } catch (NoSuchMethodException ignored) {
                    System.err.println("[PatientHome] " + ctrl.getClass().getSimpleName() + " has no setUser()");
                } catch (Exception e) {
                    System.err.println("[PatientHome] setUser() failed: " + e.getMessage());
                }
            }
    ...
    private ScrollPane buildPlaceholder(String title) {
        ...
        header.setStyle("-fx-padding:22 30;-fx-background-color:white;-fx-effect:dropshadow(gaussian,rgba(0,0,0,0.06),8,0,0,2);");
