...
public class PatientHome {
        ...
        Thread t = new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    ...
                    lblAffirmation.setText("Vous avez la force de surmonter tous les defis d'aujourd'hui.");
                });
            }
        });
        t.setDaemon(true);
        ...
        ns.setOnIncomingCall(call -> {
            try {
                ...
                utils.Notificationhelper.show("📞",
                        ...
                        "Appel entrant - Appuyez pour repondre",
    ...
    @FXML void showWellness(MouseEvent event) {
        try {
            ...
            e.printStackTrace();
            LightDialog.showError("Erreur", "Impossible de charger l'espace bien-etre.");
        }
    }
    ...
    @FXML void handleLogout(MouseEvent event) {
        if (LightDialog.showConfirmation("Deconnexion", "Voulez-vous vraiment quitter ?", "👋")) {
            try {
